...
@dataclass
class AgentReviewSegment:
    id: str
    label: str
    kind: Literal["commit", "uncommitted"]
    ...
    def to_dict(self) -> dict:
        ...
        if self.commit_hash is not None:
            d["commitHash"] = self.commit_hash
        if self.commit_message is not None:
            d["commitMessage"] = self.commit_message
        return d
...
class AgentReviewPayload:
    ...
    files: list[AgentReviewFile] = field(default_factory=list)
    segments: list[AgentReviewSegment] = field(default_factory=list)
    ...
    def to_dict(self) -> dict:
        ...
        if self.segments:
            d["segments"] = [segment.to_dict() for segment in self.segments]
        return d
