...
import { loadReviewPayloadFromSession } from "@/lib/storage/reviews";
...
export default function ReviewPage() {
  ...
  useEffect(() => {
    try {
      const activeReview = loadReviewPayloadFromSession();
      if (!activeReview) {
        setError("No payload found. Please go back and paste one.");
        return;
      }
      setPayload(asPayload(activeReview.payload));
      setSessionId(activeReview.sessionId);
    } catch (e) {
