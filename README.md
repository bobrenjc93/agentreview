...
- **Settings page** -- pick the agent (Claude Code or Codex), model, and Codex reasoning effort at `/settings` (defaults: `claude-opus-4-8` / `gpt-5.5` / Codex default); choices persist to `~/.config/agentreview/settings.json` (and browser localStorage) and apply to future `--local` runs (env overrides: `AGENTREVIEW_AGENT`, `AGENTREVIEW_MODEL`)
